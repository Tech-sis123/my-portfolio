...
export const metadata: Metadata = {
  title: "Hardware Engineering — Esabu Blessing",
  description: "Embedded Systems, PCBs, Chip Architecture, FPGA",
};

export default function HardwarePage() {
  const domainProjects = projects.filter(p => (p.domains as readonly string[]).includes("hardware"));
  const domainSkills = skillGroups.filter(g => g.label.includes("Hardware") || g.label.includes("Languages"));
    ...
    <>
      <section className="relative min-h-[70vh] flex flex-col justify-center px-6 pt-32 pb-12 max-w-[1200px] mx-auto">
        <h1 className="overflow-hidden" style={{ fontFamily: "var(--font-fraunces)", fontWeight: 300, lineHeight: 1.05 }}>
          <span className="block italic" style={{ fontSize: "clamp(3rem, 8vw, 7rem)", color: "var(--hardware)" }}>Hardware</span>
        ...
        <p className="mt-6" style={{ fontFamily: "var(--font-jetbrains)", fontSize: "clamp(0.7rem, 1.8vw, 0.9rem)", letterSpacing: "0.12em", maxWidth: "48rem" }}>
          <span style={{ color: "var(--ink)" }}>Embedded · PCBs · FPGA.</span>
        </p>
      </section>

      <div className="max-w-[1200px] mx-auto px-6">
        <section className="py-24 border-t border-[var(--hairline)]">
          <RevealOnScroll><SectionLabel ordinal="01" label="About Hardware" /></RevealOnScroll>
          <div className="grid grid-cols-1 lg:grid-cols-[1.4fr_1fr] gap-12 items-start">
            <RevealOnScroll delay={0.1}>
              <div style={{ fontFamily: "var(--font-bricolage)", fontSize: "1.05rem", lineHeight: 1.75, color: "var(--ink-dim)", fontWeight: 300 }}>
                <p>Designing embedded systems, PCBs, and FPGA solutions. Bringing low-level firmware to life for real-world impact.</p>
              </div>
            </RevealOnScroll>
            <RevealOnScroll delay={0.2}>
              <div className="relative overflow-hidden" style={{ aspectRatio: "4/5", background: "var(--ink-fainter)", border: "1px solid var(--hairline)", maxWidth: 360 }}>
                <Image
                  src="/images/hardware-photo.png"
                  alt="Hardware engineering — Esabu Blessing working on a PCB"
                  fill
            ...
            {domainSkills.map((g, gi) => (
              <RevealOnScroll key={g.label} delay={gi * 0.07}>
                <div className="flex flex-wrap items-start gap-x-6 gap-y-3 py-4 border-b border-[var(--hairline)]">
                  <div className="flex-shrink-0 w-40" style={{ fontFamily: "var(--font-jetbrains)", fontSize: "0.62rem", letterSpacing: "0.12em", color: "var(--hardware)", textTransform: "uppercase" }}>{g.label}</div>
            ...
            {domainProjects.map((p, i) => (
              <RevealOnScroll key={p.title} delay={i * 0.08}>
                <div className="card-hover p-6 h-full flex flex-col" style={{ borderRight: i % 2 === 0 ? "1px solid var(--hairline)" : "none", borderBottom: i < domainProjects.length - 2 ? "1px solid var(--hairline)" : "none" }}>
                  ...
                  <div style={{ fontFamily: "var(--font-jetbrains)", fontSize: "0.6rem", letterSpacing: "0.1em", color: "var(--hardware)", textTransform: "uppercase", marginBottom: "0.75rem" }}>{p.subtitle}</div>
                  <p style={{ fontFamily: "var(--font-bricolage)", fontSize: "0.85rem", color: "var(--ink-dim)", lineHeight: 1.6 }}>{p.desc}</p>
                  <ProjectLinks github={p.github} liveUrl={p.liveUrl} accentColor="var(--hardware)" />
                </div>
              </RevealOnScroll>
            ))}
          </div>
        ...
        <section className="py-24 border-t border-[var(--hairline)] flex justify-center">
          <a href="/docs/hardware.pdf" className="inline-block px-8 py-4 border border-[var(--hardware)] text-[var(--hardware)] hover:bg-[var(--hardware)] hover:text-[var(--bg)] transition-colors" style={{ fontFamily: "var(--font-jetbrains)", fontSize: "0.75rem", letterSpacing: "0.15em", textTransform: "uppercase" }}>
            View Hardware CV
          </a>
        </section>
      </div>
